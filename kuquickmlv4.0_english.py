...
def resource_path(relative_path):
    """ Return the file path when packaged with PyInstaller """
...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setupPreviousModelPredictionTab()  # new tab setup function
        ...
        datasplitAction.triggered.connect(self.addDataSplitTab)  # connect event
        ...
        mlpAction.triggered.connect(self.addMLPTab)  # connect event
    ...
    def loadPreviousModel(self):
        ...
        try:
            ...
            # save for use in loadUnknownSample
    ...
    def setupPreviousModelPredictionTab(self):
        # Set the tab layout and other UI components here.
        layout = QVBoxLayout()
    ...
    def runSVMClassificationCV(self):
        try:
            # SVM tab widget names (kuquickml3): self.svm_type, self.kernel_type, self.c_value
    ...
    def showSVMImportanceUnavailable(self, kernel, reducer, X_test, y_test, model, feature_names, title_prefix="SVM", task="classification"):
        ...
        try:
            ...
            perm = permutation_importance(
                wrapped,
                X_test,  # shuffle in the original feature space
                y_test,
                n_repeats=10,
                random_state=42,
                scoring=scoring
    ...
    def setupMLPTab(self):
        ...
        desc = QLabel("Maximum number of training iterations. Increase this value if the model does not converge.")
        desc.setStyleSheet(desc_style)
        ...
        desc = QLabel("Fixes the random initialization seed. Keep the same value to reproduce the same results.")
        desc.setStyleSheet(desc_style)
    ...
    def applyModelReducer(self):
        try:
            ...
            # check the currently selected model name
    ...
    def plotObservedVsPredicted(self, y_train, y_pred_train, y_test, y_pred_test, title):
        ...
        # add R^2, MSE, and RMSE information
        ax.text(0.05, 0.95,
        ...
        # add FigureCanvas to the dialog layout
        dialog_layout.addWidget(figure_canvas)
    ...
    def showSplitData(self, X_train, X_test, y_train, y_test):
        ...
        for i in range(len(X_train)):
            ...
            # process y_train (extract values only)
        ...
        for i in range(len(X_test)):
            ...
            # process y_test (extract values only)
    ...
    def show_scaled_data(self, scaled_X_df, y, headers):
        ...
        for i, row in scaled_X_df.iterrows():
            # display sample names as integers
            sample_value = row.iloc[0]
            if isinstance(sample_value, float) and sample_value.is_integer():
                formatted_sample = f"{int(sample_value)}"  # convert to integer and remove decimal places
            else:
...
def _kuquickml_copy_compare_results(self):
    ...
    if not parts:
        QMessageBox.information(self, 'Compare Models', 'There are no comparison results to copy.')
        return
...
def _kuquickml_save_compare_results(self):
    if (not hasattr(self, 'compareClassificationTable') or self.compareClassificationTable.rowCount() == 0) and (not hasattr(self, 'compareRegressionTable') or self.compareRegressionTable.rowCount() == 0):
        QMessageBox.information(self, 'Compare Models', 'There are no comparison results to save.')
        return
...
def _kuquickml_run_compare_models(self):
    ...
    if not classification_rows and not regression_rows:
        msg = 'There are no models to compare.'
        if failed:
            msg += '\\n\\n' + '\\n'.join(failed[:10])
    ...
    if failed:
        QMessageBox.warning(self, 'Compare Models', 'Failed to load model' + '\\n' + '\\n'.join(failed[:10]))
...
def _kuquickml_safe_load_csvviewer(self, filename):
    ...
    if failed_numeric_cols:
        ...
        if len(failed_numeric_cols) > 10:
            preview += f" and {len(failed_numeric_cols) - 10} more"
        QMessageBox.warning(
            self,
